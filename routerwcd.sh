...
then
	...
	then
		...
		for x in "${list[@]}"
		do
			if echo "$x" | grep -Ei "(^https?://www\.$hostset/|^https?://$hostset/)[A-Za-z0-9_-]+" | grep -Eiv "^https?://[^?=]*/(static|assets|images|resources|scripts)/" | grep -Eiv "\.(jpg|jpeg|gif|css|tif|tiff|png|ttf|woff|woff2|ico|icon|pdf|svg|txt|js|txt|webp)" | grep -q "\."
			then
