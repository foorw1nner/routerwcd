...
then
	...
	then
		...
		for x in "${list[@]}"
		do
			if echo "$x" | grep -Ei "^https?://www\.$hostset/|^https?://$hostset/" | grep -Eiv "/(static|assets|images|resources|scripts)/" | grep -qEiv "\.(jpg|jpeg|gif|css|tif|tiff|png|ttf|woff|woff2|ico|icon|pdf|svg|txt|js|txt|webp)"
			then
				...
				do
					...
					then
						...
						### PROBABILITY CALC
						probability="50"
						if curl -Lski "$x" -H "$setcookie" -H "$setauthorization" | grep -qEiv '^(X-Cache:|X-Cache-Status:|X-Drupal-Cache:|X-Joomla-Cache:|X-Varnish:|X-Magento-Cache:|X-Sucuri-Cache:|X-Edge-Cache:|CF-Cache-Status:|X-CDN-Cache:|X-Fastly-Cache:|X-Proxy-Cache:|X-Nginx-Cache:|X-Cache-Server:|X-Cache-Provider:|X-Cache-Lookup:|X-Redis-Cache:|X-Cache-Int:|X-Accel-Cache:|X-Memcached-Cache:|X-Hyper-Cache:|X-WP-Cache:|X-Page-Cache:)\s(miss|hit)|^Server-Timing:\scdn-cache;\sdesc=(hit|miss)'
						then
						...
						if curl -Lsk "$z$path_original" -H "$setcookie" -H "$setauthorization" | grep -qEi "$setmatch"
						then
